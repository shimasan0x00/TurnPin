...
**AC4 の確認方法**: OS の自動回転を **ON** にした（＝TurnPin が無ければ物理回転に追従する）状態で「縦」を適用し、端末を立てて 4 方向に回しました。加速度センサーの値が 4 方向すべてを通る間、画面は縦（rotation 0 / 1200x2000）のまま変化しませんでした。

**AC7 の確認方法**: 「起動時に自動で開始」を ON・モード「縦」で `adb reboot`。再起動後、TurnPin のアプリを開いていない状態（前面は設定アプリ）でオーバーレイ・前面サービス・常駐通知がすべて復帰し、OS の自動回転が ON のままでも画面は縦に保たれていました。
